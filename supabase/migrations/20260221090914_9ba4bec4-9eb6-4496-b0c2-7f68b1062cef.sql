
-- =============================================
-- 1. FIX SECURITY ISSUES
-- =============================================

-- Fix: order_proofs INSERT policy (admin only via has_role)
-- Fix: order_proofs DELETE policy (admin only)
-- Fix: profiles DELETE policy

-- We'll create the role system first, then use it for admin-only policies

-- =============================================
-- 2. ADMIN ROLE SYSTEM
-- =============================================

-- Create role enum
CREATE TYPE public.app_role AS ENUM ('admin', 'moderator', 'user');

-- Create user_roles table
CREATE TABLE public.user_roles (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  role app_role NOT NULL,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  UNIQUE (user_id, role)
);

-- Enable RLS
ALTER TABLE public.user_roles ENABLE ROW LEVEL SECURITY;

-- Security definer function to check roles (avoids RLS recursion)
CREATE OR REPLACE FUNCTION public.has_role(_user_id uuid, _role app_role)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT EXISTS (
    SELECT 1
    FROM public.user_roles
    WHERE user_id = _user_id
      AND role = _role
  )
$$;

-- RLS policies for user_roles
CREATE POLICY "Users can view their own roles"
  ON public.user_roles FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

CREATE POLICY "Admins can view all roles"
  ON public.user_roles FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can manage roles"
  ON public.user_roles FOR ALL
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

-- =============================================
-- 3. FIX SECURITY POLICIES
-- =============================================

-- order_proofs: Only admins can insert proofs
CREATE POLICY "Admins can insert proofs"
  ON public.order_proofs FOR INSERT
  TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

-- order_proofs: Only admins can delete proofs
CREATE POLICY "Admins can delete proofs"
  ON public.order_proofs FOR DELETE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- profiles: Users can delete their own profile
CREATE POLICY "Users can delete their own profile"
  ON public.profiles FOR DELETE
  USING (auth.uid() = user_id);

-- =============================================
-- 4. ADMIN ACCESS TO ALL TABLES
-- =============================================

-- Admins can view all orders
CREATE POLICY "Admins can view all orders"
  ON public.team_orders FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- Admins can update all orders
CREATE POLICY "Admins can update all orders"
  ON public.team_orders FOR UPDATE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- Admins can view all profiles
CREATE POLICY "Admins can view all profiles"
  ON public.profiles FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- Admins can view all roster entries
CREATE POLICY "Admins can view all roster"
  ON public.order_roster FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- Admins can view all proofs
CREATE POLICY "Admins can view all proofs"
  ON public.order_proofs FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- Admins can view all drafts
CREATE POLICY "Admins can view all drafts"
  ON public.draft_orders FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- Admins can view all templates
CREATE POLICY "Admins can view all templates"
  ON public.design_templates FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));
